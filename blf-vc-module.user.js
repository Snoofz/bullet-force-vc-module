// ==UserScript==
// @name         BLF Voice Chat Module
// @version      2024-09-09
// @description  Allows you to use voice chat on Bullet Force
// @author       Foonix, Snoofz, Ember Paws
   ...
   (function() {
    const originalFetch = window.fetch;
       ...
       window.fetch = async function(...args) {
           ...
           if (args[0].includes('https://server.blayzegames.com/OnlineAccountSystem/get-account-rolesV2.php') && options.body instanceof Blob) {
            ...
            try {
                ...
                if (data.type === "userJoin") {
                    ...
                    recentUsers.forEach(username => {
                        ...
                        tabs[3].uiTab.addButton(`Invite ${data.username} to party`, () => {
                            let party = generateRandomPartyId(selfUsername);
                            ws.send(JSON.stringify({
                                type: 'invite',
